...
#[doc = "Field `GLOBALEN` reader - Set register field GlobalEn to 1 enable the tracing and statistics collection sub-systems of the packet probe."]
pub type GlobalenR = crate::BitReader;
#[doc = "Field `GLOBALEN` writer - Set register field GlobalEn to 1 enable the tracing and statistics collection sub-systems of the packet probe."]
pub type GlobalenW<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `ACTIVE` reader - Register field Active is used to inform software that the probe is active. Probe configuration is not allowed during the active state. This bit is raised when bit GlobalEn is set, and is cleared a few cycles after setting GlobalEn to zero (probe is Idle)."]
pub type ActiveR = crate::BitReader;
impl R {
    #[doc = "Bit 0 - Set register field GlobalEn to 1 enable the tracing and statistics collection sub-systems of the packet probe."]
    #[inline(always)]
    pub fn globalen(&self) -> GlobalenR {
        GlobalenR::new((self.bits & 1) != 0)
    }
    #[doc = "Bit 1 - Register field Active is used to inform software that the probe is active. Probe configuration is not allowed during the active state. This bit is raised when bit GlobalEn is set, and is cleared a few cycles after setting GlobalEn to zero (probe is Idle)."]
    #[inline(always)]
