...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn txeie(&mut self) -> TxeieW<IntcrSpec> {
        TxeieW::new(self, 0)
    }
    #[doc = "Bit 1 - TX underrun interrupt enable"]
    #[inline(always)]
    #[must_use]
    pub fn txuie(&mut self) -> TxuieW<IntcrSpec> {
        TxuieW::new(self, 1)
    }
    #[doc = "Bit 2 - TX underrun interrupt clear\n\nWrite 1 to clear TX underrun interrupt."]
    #[inline(always)]
    #[must_use]
    pub fn txuic(&mut self) -> TxuicW<IntcrSpec> {
        TxuicW::new(self, 2)
    }
    #[doc = "Bits 4:8 - Transmit FIFO Threshold\n\nWhen the number of transmit FIFO (TXFIFO0 if TCSR=00;\n\nTXFIFO1 if TCSR=01, TXFIFO2 if TCSR=10, TXFIFO3 if TCSR=11)\n\nentries is less than or equal to this threshold, the transmit FIFO\n\nempty interrupt is triggered."]
    #[inline(always)]
    #[must_use]
    pub fn tft(&mut self) -> TftW<IntcrSpec> {
        TftW::new(self, 4)
    }
    #[doc = "Bit 16 - RX full interrupt enable"]
    #[inline(always)]
    #[must_use]
    pub fn rxfie(&mut self) -> RxfieW<IntcrSpec> {
        RxfieW::new(self, 16)
    }
    #[doc = "Bit 17 - RX overrun interrupt enable"]
    #[inline(always)]
    #[must_use]
    pub fn rxoie(&mut self) -> RxoieW<IntcrSpec> {
        RxoieW::new(self, 17)
    }
    #[doc = "Bit 18 - RX overrun interrupt clear\n\nWrite 1 to clear RX overrun interrupt."]
    #[inline(always)]
    #[must_use]
    pub fn rxoic(&mut self) -> RxoicW<IntcrSpec> {
        RxoicW::new(self, 18)
    }
    #[doc = "Bits 20:24 - Receive FIFO Threshold\n\nWhen the number of receive FIFO entries (RXFIFO0 if RCSR=00;\n\nRXFIFO1 if RCSR=01, RXFIFO2 if RCSR=10, RXFIFO3 if\n\nRCSR=11) is more than or equal to this threshold plus 1, the\n\nreceive FIFO full interrupt is triggered."]
    #[inline(always)]
    #[must_use]
    pub fn rft(&mut self) -> RftW<IntcrSpec> {
        RftW::new(self, 20)
    }
}
