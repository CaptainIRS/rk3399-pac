...
#[doc = "Field `LOCKED_BUFFER` reader - When a frame is received, this bit would be active. The\n\nCEC controller answers to all the messages with NACK\n\nuntil the CPU writes it to '0'."]
pub type LockedBufferR = crate::BitReader;
#[doc = "Field `LOCKED_BUFFER` writer - When a frame is received, this bit would be active. The\n\nCEC controller answers to all the messages with NACK\n\nuntil the CPU writes it to '0'."]
pub type LockedBufferW<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    #[doc = "Bit 0 - When a frame is received, this bit would be active. The\n\nCEC controller answers to all the messages with NACK\n\nuntil the CPU writes it to '0'."]
    #[inline(always)]
