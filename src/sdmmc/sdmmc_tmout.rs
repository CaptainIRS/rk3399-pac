...
#[doc = "Field `RESPONSE_TIMEOUT` reader - Response timeout value. Value is in number of card output clocks –cclk_out."]
pub type ResponseTimeoutR = crate::FieldReader;
#[doc = "Field `RESPONSE_TIMEOUT` writer - Response timeout value. Value is in number of card output clocks –cclk_out."]
pub type ResponseTimeoutW<'a, REG> = crate::FieldWriter<'a, REG, 8>;
#[doc = "Field `DATA_TIMEOUT` reader - Value for card Data Read Timeout; same value also used for Data Starvation by Host timeout. Value is in number of card output clocks cclk_out of selected card. Note: The software timer should be used if the timeout value is in the order of 100 ms. In this case, read data timeout interrupt needs to be disabled."]
...
impl R {
    #[doc = "Bits 0:7 - Response timeout value. Value is in number of card output clocks –cclk_out."]
    #[inline(always)]
    pub fn response_timeout(&self) -> ResponseTimeoutR {
        ResponseTimeoutR::new((self.bits & 0xff) as u8)
    }
    #[doc = "Bits 8:31 - Value for card Data Read Timeout; same value also used for Data Starvation by Host timeout. Value is in number of card output clocks cclk_out of selected card. Note: The software timer should be used if the timeout value is in the order of 100 ms. In this case, read data timeout interrupt needs to be disabled."]
    #[inline(always)]
