...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn efuse_csb(&mut self) -> EfuseCsbW<CtrlSpec> {
        EfuseCsbW::new(self, 0)
    }
    #[doc = "Bit 1 - efuse turn on the array for read or program access (active high) :\n\nSTROBE"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_strobe(&mut self) -> EfuseStrobeW<CtrlSpec> {
        EfuseStrobeW::new(self, 1)
    }
    #[doc = "Bit 2 - efuse turn on sense amplifier and load data into latch (active\n\nhigh) : LOAD"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_load(&mut self) -> EfuseLoadW<CtrlSpec> {
        EfuseLoadW::new(self, 2)
    }
    #[doc = "Bit 3 - efuse program enable (active low) : PGENB"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_pgenb(&mut self) -> EfusePgenbW<CtrlSpec> {
        EfusePgenbW::new(self, 3)
    }
    #[doc = "Bit 4 - efuse pass 1.8V program voltage to internal for program(active\n\nhigh) : PS"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_ps(&mut self) -> EfusePsW<CtrlSpec> {
        EfusePsW::new(self, 4)
    }
    #[doc = "Bit 5 - efuse power down enable (active high) : PD"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_pd(&mut self) -> EfusePdW<CtrlSpec> {
        EfusePdW::new(self, 5)
    }
    #[doc = "Bit 6 - efuse read trip point setting, MR = L for normal read mode; MR =\n\nH for margin read1\n\nmode : MR"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_mr(&mut self) -> EfuseMrW<CtrlSpec> {
        EfuseMrW::new(self, 6)
    }
    #[doc = "Bit 7 - efuse redundancy enable(active low) : RSB"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_rsb(&mut self) -> EfuseRsbW<CtrlSpec> {
        EfuseRsbW::new(self, 7)
    }
    #[doc = "Bit 8 - efuse redundancy information row select (active high) : RWL"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_rwl(&mut self) -> EfuseRwlW<CtrlSpec> {
        EfuseRwlW::new(self, 8)
    }
    #[doc = "Bit 9 - efuse strobe control software mode select, active high"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_strobe_sft_sel(&mut self) -> EfuseStrobeSftSelW<CtrlSpec> {
        EfuseStrobeSftSelW::new(self, 9)
    }
    #[doc = "Bits 16:25 - efuse address pins :A\\[9:0\\]"]
    #[inline(always)]
    #[must_use]
    pub fn efuse_addr(&mut self) -> EfuseAddrW<CtrlSpec> {
